...
在 XML 控制程序中，同一个变量可能在多个位置被引用（作为输入、输出、参数等）。早期版本中，每次引用都会生成一个独立的变量记录，导致输出中存在重复。
...
1. **主要去重（提取器层）**：
   - 在 `MainExtractor` 完成变量提取后，自动按 `name` 字段去重
   - 同一变量的多个提取记录会被合并为一条记录
   - 所有 evidence 信息都会保留，不会丢失溯源数据

2. **防御性去重（格式化器层）**：
   - 在 `UnifiedFormatter` 生成统一输出时，再次检查是否有重复
   - 如果检测到重复，记录警告日志并自动去重
   - 作为安全网，防止未来代码变更引入重复
...
- **变量数据**：保留第一个记录的数据（假设同名变量的属性应该相同）
- **Evidence 数据**：
  - 合并所有记录的 evidence 信息
  - 对于重复的键，添加序号后缀（如 `e_name_2`, `e_name_3`）
  - 添加 `merged_count` 字段，记录合并的来源数量
...
```python
# 启用去重（默认）
extractor = MainExtractor(deduplicate_variables=True)

# 禁用去重（用于调试）
extractor = MainExtractor(deduplicate_variables=False)
```
...
- `19_CCS02.xml`: 53 个 → 38 个（去除 15 个重复，28% 冗余率）
- `ECS03.xml`: 40 个 → 21 个（去除 19 个重复，48% 冗余率）
- 全量数据集：1178 个文件，平均冗余率约 25-30%
...
```python
import json
data = json.load(open('output/unified/19_CCS02_unified_clean.json', 'r', encoding='utf-8'))
vars_list = data['D']['vars']
names = [v['name'] for v in vars_list]
print(f'总变量数: {len(vars_list)}, 唯一变量数: {len(set(names))}')
# 应该输出相同的数字，如: 总变量数: 38, 唯一变量数: 38
```

### Q9: 去重后 evidence 信息会丢失吗？（NEW）

不会。同一变量的所有 evidence 信息都会被合并保留，并添加 `merged_count` 字段标记合并数量。

### Q10: 如何禁用去重？（NEW）

如果需要保留所有重复记录（用于调试），可以在代码中设置：
